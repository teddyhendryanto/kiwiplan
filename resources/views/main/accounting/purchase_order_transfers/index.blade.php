...
@section('breadcrumb')
  <div id="#breadcrumb">
    <ol class="breadcrumb">
      ...
      <li><a href="{{ route('purchase_order_transfers.index') }}" class="active">Transfer</a></li>
    </ol>
  </div>
@endsection
    ...
    <div class="col-md-12">
      <div id="panel-info" class="panel panel-default">
        <div class="panel-heading">
          <h3 class="panel-title">Purchase Order Transfer</h3>
        </div>
        <div id="panel-body" class="panel-body">
          <div class="row">
            <div class="col-md-12">
              <form id="form-filter" class="form" role="form" method="GET" action="{{ route('purchase_order_transfers.show', env('TARGET_SITE_ID')) }}">
                {{ csrf_field() }}
                @if(isset($data))
                ...
                <div class="row">
                  ...
                  <div class="col-md-3 col-md-offset-3 text-right">
                    <a href="{{ route('purchase_order_transfers.create') }}" class="btn btn-success">
                      Buat Baru
                    </a>
                  </div>
                </div>
                    ...
                    <thead class="f12">
                      ...
                      @foreach ($transfers as $data)
                        <tr>
                          ...
                          <td class="text-center w10">{{ $data->purchase_order->site->short_name }} </td>
                          <td class="text-center w30">{{ $data->purchase_order->supplier->full_name }}</td>
                          <td class="text-center w20">{{ $data->purchase_order->po_num }}</td>
                          <td class="text-center w20">{{ date('Y-m-d', strtotime($data->purchase_order->po_date)) }}</td>
                          <td class="text-center w15">
                            <a href="{{ route('purchase_orders.print', $data->purchase_order->id) }}" class="btn btn-xs btn-default" target="_blank">
